...
public class ParserTest {
	 ...
	 */
	@Test
	public void testTheNewFancyJava8Features() throws IOException {
		File file = new File(srcTestResources, "Java8FeatureTest.java");
	...
	private String buildTokenString(jplag.Structure oldStruct, boolean withDetails) {
		...
		for (int i = 0; i < oldStruct.size(); i++) {
			sb.append(jplag.java18.JavaToken.type2string(oldStruct.tokens[i].type));
			if (withDetails) {
	...
	private String parseWithJ7Parser(File javaFile, boolean withDetails) {
		...
		jplag.java18.Parser newParser = new Parser();
